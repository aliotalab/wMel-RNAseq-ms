...
## Day 4 Samples
targets_carcass4 <- read_tsv("~/Desktop/mosquito_rnaseq/res_v_sus_zikvinfect/carcass4/carcass4_studydesign.txt")
targets_midgut4 <- read_tsv("~/Desktop/mosquito_rnaseq/res_v_sus_zikvinfect/midgut4/midgut4_studydesign.txt")

path_carcass4 <- file.path("carcass4", targets_carcass4$sample_pool, "abundance.tsv")
path_midgut4 <- file.path("midgut4", targets_midgut4$sample_pool, "abundance.tsv")
...
## Day 7 Samples
targets_carcass7 <- read_tsv("~/Desktop/mosquito_rnaseq/res_v_sus_zikvinfect/carcass7/carcass7_studydesign.txt")
targets_midgut7 <- read_tsv("~/Desktop/mosquito_rnaseq/res_v_sus_zikvinfect/midgut7/midgut7_studydesign.txt")

path_carcass7 <- file.path("carcass7", targets_carcass7$sample_pool, "abundance.tsv")
path_midgut7 <- file.path("midgut7", targets_midgut7$sample_pool, "abundance.tsv")
...
#adapted from Zamanian lab code
...
vplot <- ggplot(deg) +
  ...
  #geom_point(data = dplyr::filter(deg, is_wolb == "wolb_gene" & sig == "yes"), color = "cyan", size = 1, alpha = 0.5) +
  #geom_point(data = dplyr::filter(deg, is_toll == "toll_gene" & sig == "yes"), color = "yellow", size = 1, alpha = 0.5) +
  #geom_point(data = dplyr::filter(deg, is_jak == "jak_gene" & sig == "yes"), color = "purple", size = 1, alpha = 0.85) +
...
#write_csv2(vsd_degs, file = "./resvsus_zikvinfect_counts_GSEA.csv")
vsd_degs <- vsd_degs %>%
...
counts$wmel_status <- factor(counts$wmel_status, levels = c("WZ", "TZ"))
counts$dpf <- factor(counts$dpf, levels = c("4D", "7D"))
counts$tissue <- factor(counts$tissue, levels = c("MG", "C"))

c4genenames <- read_csv("./immune_lists/c4genenames.csv")
colnames(c4genenames) <- c("gene_id", "gene_name")
counts <- merge(counts, c4genenames)

grouplabs <- c("COL.tet, carcass", "COL.tet, midgut", "COL.wMel, carcass", "COL.wMel, midgut")
...
heatmap <- ggplot(counts, aes(sample, gene_name)) +
  ...
  #geom_vline(xintercept = 17.5, linetype="dashed", color = "black", size=0.5) +
  xlab("") + 
  ylab("") + 
  theme_bw() +
  theme(
    ...
    axis.text.y = element_text(face = "italic", size = 5, angle =0, hjust=1),
...
temp_text <- data.frame(
  label = c("alpha-actinin"),
  tissue_type = c("carcass"),
  x = c("log2FC_7"),
  y = c(24.5)
)
...
temp_plot <- ggplot(data = dplyr::filter(deg_temporal, pattern %in% c("constitutively increased","constitutively decreased")), text = paste("Symbol:", gene_id)) +
  aes(y = log2FoldChange, x = factor(as.character(log2FC_day))) +
  ...
  geom_text(data = temp_text,
            mapping = aes(x = x, y = y, label = label), size =2, nudge_x = 0.25) +
...
figure_4.6 <- plot_grid(vplot, heatmap, temp_plot, labels = c("A.","B.", "C."), nrow = 1, align = "tblr")
figure_4.6
